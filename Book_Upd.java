 ...
 * @author maruthi
 */
public class Book_Upd extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String query="";
        int s=0;
        if((bname != null && !bname.isEmpty() && !bname.equals(" "))){
            s=st.executeUpdate("update cbooks set title='"+bname+"' where id='"+bid+"'");
        }
        if((aname != null && !aname.isEmpty() && !aname.equals(" "))){
            s=st.executeUpdate("update cbooks set author='"+aname+"' where id='"+bid+"'");
        }
        if((category != null && !category.isEmpty() && !category.equals("null"))){
            s=st.executeUpdate("update cbooks set category='"+category+"' where id='"+bid+"'");
        }
        if((publications != null && !publications.isEmpty() && !publications.equals(" "))){
            s=st.executeUpdate("update cbooks set publications='"+publications+"' where id='"+bid+"'");
        }
        if((photo != null && !photo.isEmpty() && !photo.equals(" "))){
            s=st.executeUpdate("update cbooks set coverImage='"+photo+"' where id='"+bid+"'");
        }
        if((status != null && !status.isEmpty() && !status.equals("null"))){
            s=st.executeUpdate("update cbooks set status='"+status+"' where id='"+bid+"'");
        }
...
"             <center><h1 style=\"color:green; \"><b><h1> Book with ID: "+ bid+" Successfully Updated </h1></center>\n" +
"        </div>");
            }
            else{
                out.println(""
                        + "<div id='msg' style='box-shadow: 0 0 7px red;border: 2px solid red ;'>\n" +
"             <center><h1 style=\"color:red; \"><b><h1>Book with ID: "+bid+" Unsuccessfull !!!!</h1></h1></center>\n" +
"        </div> ");
            }
           // out.println(bid+" "+bname+" "+aname+" "+category+" "+publications+" "+photo+" "+status);
